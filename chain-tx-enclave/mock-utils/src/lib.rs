...
pub fn unseal(payload: &[u8]) -> TxWithOutputs {
    let bytes = payload.iter().map(|b| b ^ SEAL_KEY).collect::<Vec<_>>();
    TxWithOutputs::decode(&mut bytes.as_slice()).unwrap()
}
