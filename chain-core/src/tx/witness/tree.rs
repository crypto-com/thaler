...
use secp256k1::constants::{COMPACT_SIGNATURE_SIZE, COMPRESSED_PUBLIC_KEY_SIZE};
use secp256k1::{key::PublicKey, Secp256k1, Signature};
...
pub type RawPubkey = (u8, [u8; COMPRESSED_PUBLIC_KEY_SIZE - 1]);
/// TODO: custom serializers + eq etc. impls
...
pub fn pk_to_raw(secp: &Secp256k1, pk: PublicKey) -> RawPubkey {
    let compressed = &pk.serialize_vec(secp, true);
    let mut r = [0; 32];
    r.copy_from_slice(&compressed[1..COMPRESSED_PUBLIC_KEY_SIZE]);
    (compressed[0], r)
}

/// conversion for custom wrappers
pub fn sig_to_raw(secp: &Secp256k1, sig: Signature) -> RawSignature {
    let compressed = &sig.serialize_compact(secp);
    let mut r1 = [0; 32];
