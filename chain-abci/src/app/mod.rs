...
impl<T: EnclaveProxy> abci::Application for ChainNodeApp<T> {
    ...
    fn check_tx(&mut self, req: &RequestCheckTx) -> ResponseCheckTx {
        ...
        match self.process_tx(req, false) {
            Ok(_) => {
                resp.set_code(0);
            }
            Err(msg) => {
    ...
    fn deliver_tx(&mut self, req: &RequestDeliverTx) -> ResponseDeliverTx {
        ...
        match self.process_tx(req, true) {
...
impl<T: EnclaveProxy> ChainNodeApp<T> {
    fn process_tx(
        ...
        let mtxaux = validate_tx_req(
            &staking_getter!(self, account_root, deliver),
        ...
        mtxaux.and_then(|(txaux, action)| {
            let (fee, maccount) = match &action {
                TxAction::Enclave(action) => execute_enclave_tx(
                    &mut staking_store!(self, account_root, deliver),
                    &mut kv_store!(self, deliver),
                ...
                TxAction::Public(tx) => execute_public_tx(
                    ...
                    if deliver {
                        ...
                        self.mempool_state.as_mut().unwrap()
                    },
                    &tx,
                    &extra_info,
                )?,
