...
use client_common::{
    BlockHeader, Error, ErrorKind, PrivateKey, Result, ResultExt, SecureStorage, Transaction,
};
...
impl<'a, S: SecureStorage, C: Client, D: TxDecryptor> WalletSyncerImpl<'a, S, C, D> {
    ...
    fn handle_batch(&mut self, blocks: NonEmpty<BlockHeader>) -> Result<()> {
    ...
    fn sync(&mut self) -> Result<()> {
            ...
            .into_iter()
        {
            ...
            for (block, block_result, state) in izip!(
                ...
                let block = prepare_block_header(&self.wallet, &block, &block_result)?;
                self.update_progress(block.block_height);
                batch.push(block);
            }
    ...
    fn fast_forward_status(&self, status: &Status) -> Result<Option<BlockHeader>> {
    ...
    fn fast_forward_block(&mut self, block: &Block) -> Result<Option<BlockHeader>> {
...
fn decrypt_staking_transactions(
...
fn prepare_block_header(
    ...
    let staking_transactions =
        decrypt_staking_transactions(&block_result, &wallet.staking_addresses(), block)?;

    let enclave_transaction_ids = if block_filter.check_view_key(&wallet.view_key.clone().into()) {
